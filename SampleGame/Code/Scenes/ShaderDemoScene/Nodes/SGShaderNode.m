...
- (id) initWithShaderNamed:(NSString*)shaderName
{
    ...
    self = [super initWithSize:size andColor:[B3DColor whiteColor]];
    if (self)
    {
...
- (void) update
{
    ...
    [shader setVector2Value:GLKVector2Make(self.size.width, self.size.height) forUniformNamed:@"resolution"];
    [shader setFloatValue:time forUniformNamed:@"time"];
    [shader setVector2Value:GLKVector2Make(self.position.x, self.position.y) forUniformNamed:@"u_offset"];
